...
/**
 * Created by BCLZzy on 2017/10/16.
 * 圆角相对布局
 */
...
public class CornerRelativeLayout extends RelativeLayout {
    ...
    public CornerRelativeLayout(Context context) {
        this(context, null);//注意这里用this
    }

    public CornerRelativeLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);//注意这里用this
    }

    public CornerRelativeLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CornerRelativeLayout);
        mRoundAsCircle = ta.getBoolean(R.styleable.CornerRelativeLayout_round_as_circle, false);
        mStrokeColor = ta.getColor(R.styleable.CornerRelativeLayout_stroke_color, Color.WHITE);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.CornerRelativeLayout_stroke_width, 0);
        int roundCorner = ta.getDimensionPixelSize(R.styleable.CornerRelativeLayout_round_corner, 0);
        int roundCornerTopLeft = ta.getDimensionPixelSize(
                R.styleable.CornerRelativeLayout_round_corner_top_left, roundCorner);
        int roundCornerTopRight = ta.getDimensionPixelSize(
                R.styleable.CornerRelativeLayout_round_corner_top_right, roundCorner);
        int roundCornerBottomLeft = ta.getDimensionPixelSize(
                R.styleable.CornerRelativeLayout_round_corner_bottom_left, roundCorner);
        int roundCornerBottomRight = ta.getDimensionPixelSize(
                R.styleable.CornerRelativeLayout_round_corner_bottom_right, roundCorner);
